...
for outFile in sys.argv[2:]:
    ...
    if sys.argv[1] == 'variable':
        label = 'instructions per reference cycle (ipc)'
        dir = 'var'
        ...
        size_slowdown_factor = 1.1 ** np.log2(int(l1_size) / size_baseline) 
        assoc_slowdown_factor = 1.05  **  np.log2(int(l1_assoc) / asssoc_baseline)
...
ax1.set_ylabel("$IPC$")
line1 = ax1.plot(ipc_Axis, label=label, color=color,marker='x')
...
plt.title("[L2 (" + title + " cycle) - BENCHMARK: " + benchmark + "]  IPC vs MPKI")
lgd = plt.legend(lns, labs)
lgd.draw_frame(False)
outputIMGName = "../../report/graphs/L2/" + dir + "/" + benchmark + ".png"
