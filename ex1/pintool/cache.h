    ...
    if (!l1Hit) {
        ...
        if (!l2Hit) {
	        ...
	        for (UINT32 i=0; i < _l2_prefetch_lines; i++) {
                ...
                // Solo code:
                // Let's start by splitting out address like a pro
                SplitAddress(prefetch_addr, L2LineShift(), L2SetIndexMask(), l2Tag, l2SetIndex);
                // Now, based on the index we got, we must find on which set (remember associativity) it must lie
                SET & l2Set = _l2_sets[l2SetIndex];
                // Do we care to look up if we already have this in L2 Cache? -YES!
                l2Hit = l2Set.Find(l2Tag);
                if (!(l2Hit)) {
                    CACHE_TAG l2_replaced = l2Set.Replace(l2Tag);
